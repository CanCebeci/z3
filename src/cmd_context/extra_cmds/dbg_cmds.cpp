...
UNARY_CMD(add_cgr_listener_cmd, "add-cgr-listener", "<term>", "Register a congruence-root listener term for failure diagnostics", CPK_EXPR, expr *, {
    solver * s = ctx.get_solver();
    if (!s)
        throw cmd_exception("solver is not available");
    s->add_cgr_listener(arg);
});
...
void install_dbg_cmds(cmd_context &ctx) {
    ...
    ctx.insert(alloc(add_cgr_listener_cmd));
