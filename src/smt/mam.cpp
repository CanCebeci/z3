        ...
        enode * get_first_f_app(func_decl * lbl, unsigned num_expected_args, enode * curr) {
            ...
            do {
                if (curr->get_decl() == lbl  && curr->get_num_args() == num_expected_args) {
                    ...
                    // (which is practically happenstance), the generation number should be calculated as if curr
                    // is a match.
                    // (I am not sure why the is_cgr() check is necessary at all. I am assuming this is an invariant that needs to be maintained)
                    // TODO: make sure this all plays well with used_enodes for tracing purposes.
